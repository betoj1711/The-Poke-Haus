...
export function SiteHeader(){
  ...
  return <><header className="header shopHeader"><div className="container nav">
    ...
    <nav className="navlinks" aria-label="Main navigation"><Link href="/#inventory">Shop Cards</Link><Link href="/#inventory">New Arrivals</Link><Link href="/#shop">Collections</Link><Link href="/sell">Sell Bulk</Link></nav>
  ...
  </div>{open&&<nav className="mobileNav"><a href={EBAY} target="_blank" rel="noopener noreferrer">Shop eBay <ArrowUpRight size={17}/></a><Link href="/sell" onClick={()=>setOpen(false)}>Sell Bulk</Link><Link href="/faq" onClick={()=>setOpen(false)}>FAQ</Link><Link href="/contact" onClick={()=>setOpen(false)}>Contact</Link></nav>}</header>
